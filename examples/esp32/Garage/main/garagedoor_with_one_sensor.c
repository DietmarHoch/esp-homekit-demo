...
#define OPEN_CLOSE_DURATION 28

//static gpio_num_t RELAY1_PIN = GPIO_NUM_2;
//static gpio_num_t RELAY2_PIN = GPIO_NUM_2;
//static gpio_num_t REED_PIN_CLOSED_SENSOR = 14;

//   www.iotflair.com
static gpio_num_t RELAY1_PIN = GPIO_NUM_21;
static gpio_num_t RELAY2_PIN = GPIO_NUM_22;
static gpio_num_t REED_PIN_CLOSED_SENSOR = GPIO_NUM_5;
...
homekit_value_t gdo_obstruction_get() {
    //printf("returning obstruction state.'%s'\n", obstruction);
    //return HOMEKIT_BOOL(obstruction);
    printf("returning obstruction state:'%d'\n", false);
...
void current_door_state_update_from_sensor() {
    ...
    contact_sensor_state_t sensor_state = gpio_get_level(REED_PIN_CLOSED_SENSOR);
    printf("Contact sensor state'%d'......\n", sensor_state);
    switch (sensor_state) {
        case CONTACT_CLOSED:
...
homekit_value_t gdo_current_state_get() {
    ...
    //todo current door state when starting the server
...
void timer_event_fired( TimerHandle_t xTimer )
{
   ...
   // ESP_LOGI(TIMERTAG,"timer fired!!!");
    ...
    if (current_door_state == HOMEKIT_CHARACTERISTIC_CURRENT_DOOR_STATE_OPENING && contact_sensor_state_get(REED_PIN_CLOSED_SENSOR)==1) {
        //ESP_LOGI(TIMERTAG,"Garagedoor should be open now !!!");
        current_door_state_update_from_sensor();
        //current_state_set(HOMEKIT_CHARACTERISTIC_CURRENT_DOOR_STATE_OPEN);
    }
    
     else if (current_door_state == HOMEKIT_CHARACTERISTIC_CURRENT_DOOR_STATE_OPENING && contact_sensor_state_get(REED_PIN_CLOSED_SENSOR)==0){
    ...
    else if (current_door_state == HOMEKIT_CHARACTERISTIC_CURRENT_DOOR_STATE_CLOSING && contact_sensor_state_get(REED_PIN_CLOSED_SENSOR)==1){
...
static void gpio_task_example(void* arg)
{
    ...
    for(;;) {
        if(xQueueReceive(gpio_evt_queue, &io_num, portMAX_DELAY)) {
            ...
            if ((gpio_get_level(io_num)) != laststate)
            {
                printf("last state is different then current state\n");
                vTaskDelay(200 / portTICK_PERIOD_MS); //debounce time
                if ((gpio_get_level(io_num)) != laststate) {
                    printf("last state is different then current state after debounce\n");
                    contact_sensor_state_changed(io_num,gpio_get_level(io_num));
                    laststate = gpio_get_level(io_num);
                    //test for reed contact --- set relay_2 on when closed
                ...
                printf("last state is same then current state ... do nothing\n");
...
void app_main(void) {
    ...
    btn_config.pin_bit_mask = (1 << REED_PIN_CLOSED_SENSOR); //Bitmask
    ...
    gpio_isr_handler_add(REED_PIN_CLOSED_SENSOR, gpio_isr_handler, (void*) REED_PIN_CLOSED_SENSOR); //Add handler of interrupt
    printf("Interrupt configured from main\n");

    
    //xTaskCreate(&task_sensor_interrupt, "sensor_interrupt", 2048, NULL, 5, NULL);
    printf("Using Sensor at GPIO%d.\n", REED_PIN_CLOSED_SENSOR);
